...
export default function Admin({ user }: AdminProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      setMessagesPagination({
        ...
        limit: messagesRes.data.limit,
        total: messagesRes.data.total,
        totalPages: messagesRes.data.totalPages,
      });
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Messages uploaded successfully! ${response.data.count} messages added to ${response.data.batchName}`,
      );
      fetchData();
      setFile(null);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (
      !manualMessage.category ||
      !manualMessage.activity ||
      !manualMessage.affirmation
    ) {
    ...
    try {
      ...
      toast.success(
        `Message sent successfully! ${response.data.emailCount} emails, ${response.data.smsCount} SMS sent to ${response.data.totalSubscribers} subscribers`,
      );
      setManualMessage({
        category: "",
        activity: "",
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-20"} bg-gradient-to-b from-slate-900 to-indigo-900 min-h-screen transition-all duration-300 flex flex-col`}
      >
        {/* Logo */}
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-8">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setTracker({ ...tracker, currentBatch: e.target.value })
                      }
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    />
                  </div>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            {user.stripeCustomerId ? (
                              <span className="text-xs text-indigo-600">
                                {user.stripeCustomerId.slice(0, 8)}...
                              </span>
                            ) : (
                              "-"
                            )}
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <div className="p-4 flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-900">
                  Subscriptions ({users.filter((u) => u.isSubscribed).length})
                </h2>
                <button
                  onClick={fetchData}
                  className="flex items-center space-x-2 text-indigo-600 hover:text-indigo-800"
          ...
          {activeTab === "send-message" && (
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <h3 className="text-xl font-bold text-gray-900 mb-6">
                Send Manual Message to Subscribers
              </h3>

              {/* Select from existing messages */}
              <div className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Or select from existing messages (Excel uploads)
                </label>
                        ...
                        .map((message) => (
                          ...
                          >
                            <div className="font-medium text-gray-900">
                              {message.category} - Day {message.dayNumber}
                            </div>
                        ...
                        {users
                          ...
                          .map((user) => (
                            <div
                              ...
                              onClick={() => {
                                if (
                                  !manualMessage.selectedUsers.includes(
                                    user._id,
                                  )
                                ) {
                                  setManualMessage({
                            ...
                            >
                              ...
                              <div className="text-sm text-gray-500">
                                {user.name} - {user.subscriptionType} -{" "}
                                {user.isSubscribed
                                  ? "Subscribed"
                                  : "Not Subscribed"}
                              </div>
                            ...
                            <button
                              ...
                              onClick={() =>
                                setManualMessage({
                                  ...manualMessage,
                                  selectedUsers:
                                    manualMessage.selectedUsers.filter(
                                      (id) => id !== userId,
                                    ),
                                })
                              }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setManualMessage({
                        ...manualMessage,
                        targetSubscriptionType: e.target.value,
                      })
                    }
                  ...
                  {manualMessage.selectedUsers.length > 0 && (
                    <p className="text-sm text-gray-500 mt-1">
                      Specific users selected - target filter disabled
                    </p>
                  )}
                </div>
                <button
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
                ...
                <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-4 text-white">
                  ...
                  <p className="text-2xl font-bold">
                    {deliveryPagination.total > 0
                      ? Math.round(
                          (deliveries.filter((d) => d.status === "sent")
                            .length /
                            deliveryPagination.total) *
                            100,
                        )
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {deliveries.map((delivery) => (
                        <tr key={delivery._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            {new Date(delivery.sentDate).toLocaleDateString()}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${
              ...
              {deliveryPagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-6 pt-6 border-t border-gray-200">
                  ...
                  <span className="text-sm text-gray-600">
                    Page {deliveryPagination.page} of{" "}
                    {deliveryPagination.totalPages}
                  </span>
                  <button
                    onClick={() =>
                      setDeliveryPagination({
                    ...
                    disabled={
                      deliveryPagination.page === deliveryPagination.totalPages
                    }
                    className="px-4 py-2 border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50"
                  >
                    Next
          ...
          {activeTab === "messages" && (
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">
                  Messages ({messagesPagination.total})
                </h3>
                <div className="flex items-center space-x-3">
              ...
              {messagesPagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-6 pt-6 border-t border-gray-200">
                  ...
                  <span className="text-sm text-gray-600">
                    Page {messagesPagination.page} of{" "}
                    {messagesPagination.totalPages} ({messagesPagination.total}{" "}
                    total)
                  </span>
                  <button
                    onClick={() =>
